...
function parse (tree, options) {
	var target = CallGraph.findTarget(tree, options['target']);
...
var CallGraph = {
	findTarget: function cg_findTarget(tree, target) {
		var result = null;
		if (!tree ||
			typeof(tree) === 'boolean' ||
		    typeof(tree) === 'string' ||
			...
			for (var i = 0; i < tree.length; i++) {
				result = this.findTarget(tree[i], target);
				if (result)
					break;
			}
		} else {
			for (var prop in tree) {
				result = this.findTarget(tree[prop], target);
				if (result != null)
					break;
			}
		}
